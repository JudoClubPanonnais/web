    ...
    <footer className="bg-[#0f1f33] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-4">
              <Image src="/logo-jcp.jpg" alt="JCP" width={48} height={48} className="rounded-full" />
              <div>
                <div className="font-bold text-white">Judo Club Panonnais</div>
                <div className="text-gray-400 text-sm">Bras Panon — La Réunion</div>
              </div>
            </div>
            <p className="text-gray-400 text-sm leading-relaxed">
              Fondé en 2017. Affilié FFJDA. Plus de 80 pratiquants de 5 à 70 ans.
              Le tatami comme levier de changement social.
            </p>
            <div className="flex gap-3 mt-4">
              <a href="https://www.facebook.com/judoclubpanonnais" target="_blank" rel="noopener"
              ...
              <a href="https://www.instagram.com/judoclubpanonnais" target="_blank" rel="noopener"
                className="w-9 h-9 rounded-lg bg-white/10 hover:bg-orange-500 flex items-center justify-center transition-colors">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              ...
              <a href="https://www.youtube.com/@judoclubpanonnais" target="_blank" rel="noopener"
                className="w-9 h-9 rounded-lg bg-white/10 hover:bg-orange-500 flex items-center justify-center transition-colors">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* Causes */}
          <div>
            <h4 className="font-semibold mb-4 text-white">Nos causes</h4>
            <ul className="space-y-2 text-sm text-gray-400">
              {[
                ...
                ['/causes/decouverte-ailleurs', '✈️ Découverte de l\'ailleurs'],
              ].map(([href, label]) => (
                <li key={href}>
                  <Link href={href} className="hover:text-orange-400 transition-colors">{label}</Link>
                </li>
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm mb-4">
              Inscrivez-vous à notre newsletter pour suivre nos actions.
            </p>
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubscribe} className="flex flex-col gap-2">
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  ...
                  className="w-full px-4 py-2.5 bg-white/10 border border-white/20 rounded-xl text-white placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                <button type="submit" disabled={status === 'loading'}
                  className="btn-primary w-full text-sm py-2.5">
                  {status === 'loading' ? 'Inscription...' : 'S\'inscrire'}
                </button>
                {status === 'error' && <p className="text-red-400 text-xs">Une erreur est survenue.</p>}
              </form>
            )}
          </div>
        </div>
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-gray-500">
          <p>© 2026 Judo Club Panonnais — Association loi 1901 — SIRET : 832 172 753 00010</p>
